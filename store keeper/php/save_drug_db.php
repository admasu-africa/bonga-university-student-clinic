	...
	if(isset($_POST['name'])){
		...
		for ($i=0; $i < count($drug_name); $i++) { 
			...
			if($drug_name_clear != '' && $drug_qty_clear !='' && $drug_measure_clear != '' && $drug_batch_no_clear != '' && $drug_expire_date_clear != ''){
				$sql .= "INSERT INTO drug_store(drug_name, quantity, measure, batch_no, expire_date,supplier_id,registered_date,registered_by) VALUES('$drug_name_clear','$drug_qty_clear','$drug_measure_clear','$drug_batch_no_clear','$drug_expire_date_clear',1,'$date','$pharmacist');";
			}
			else{
